...
import { LiquidityTonRequest } from "../dto/liquidity-ton-request-dto";
 ...
 *
 * POST /api/liquidity_ton
 */
export const liquidityTon: HttpResponseResolver = async ({ request }) => {
  try {
    ...
    const body = LiquidityTonRequest.parse(await request.json());
    ...
    const txsParams = await Promise.all([
      ...
      router.getProvideLiquidityTonTxParams({
        ...
        sendAmount: toNano(body.ptonSendAmount),
        otherTokenAddress: body.jettonAddress,
        minLpOut: body.ptonMinLpOut,
      ...
      router.getProvideLiquidityJettonTxParams({
        userWalletAddress: senderAddress,
        sendTokenAddress: body.jettonAddress,
        sendAmount: toNano(body.jettonSendAmount),
        otherTokenAddress: new pTON.v1().address,
        minLpOut: body.jettonMinLpOut,
        queryId: getNextQueryId(),
      }),
    ]);
