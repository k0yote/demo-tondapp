...
import { SwapJettonToJettonRequest } from "../dto/swap-jetton-to-jetton-request-dto";
 ...
 *
 * POST /api/swap_jetton_to_jetton
 */
export const swapJettonToJetton: HttpResponseResolver = async ({ request }) => {
  try {
    ...
    const body = SwapJettonToJettonRequest.parse(await request.json());
    // forward value for the message to the wallet
    const walletForwardValue = toNano("0.175");
    ...
    const txParams = await router.getSwapJettonToJettonTxParams({
      ...
      offerJettonAddress: body.offerJettonAddress,
