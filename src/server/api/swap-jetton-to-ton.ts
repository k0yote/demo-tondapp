...
import { SwapJettonToTonRequest } from "../dto/swap-jetton-to-ton-request-dto";
 ...
 *
 * POST /api/swap_jetton_to_ton
 */
export const swapJettonToTon: HttpResponseResolver = async ({ request }) => {
  try {
    ...
    const body = SwapJettonToTonRequest.parse(await request.json());
    // forward value for the message to the wallet
    const walletForwardValue = toNano("0.125");
