...
import "./style.scss";
import { SwapJettonToTonRequestDto } from "../../server/dto/swap-jetton-to-ton-request-dto"; // Updated DTO
import { TonProofDemoApi } from "../../TonProofDemoApi";

export const JettonSwapTonDemo = () => {
  ...
  const handleGetJettonSwapTonFormClick = () => {
    setShowForm(true);
  };
  ...
  const handleJettonSwapTonSubmitClick = async () => {
    // Prepare faucet request DTO
    const swapJettonToTon: SwapJettonToTonRequestDto = {
    ...
    try {
      ...
      const response = await TonProofDemoApi.swapJettonToTon(swapJettonToTon);
  ...
  return (
    <div className="swap-jetton-to-ton-demo">
      <h3>Swap jetton to pTON</h3>
      {wallet ? (
        <>
          <button onClick={handleGetJettonSwapTonFormClick}>Send Swap</button>
          {showForm && (
            <div className="modal">
              <div className="modal-content">
                ...
                <div className="button-container">
                  <button onClick={handleJettonSwapTonSubmitClick}>
                    Submit Swap Jetton to pTON Request
                  </button>
                </div>
              </div>
            </div>
